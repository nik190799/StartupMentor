...
(function ($) {
    ...
    var CartStore = function(name, config) {
        this.config = $.extend({}, defaultConfig, config);
        this.init(name, this.config);
        this.queryService(false);

        var that = this;
        ContextHub.eventing.on(ContextHub.Constants.EVENT_STORE_UPDATED + ":segmentation", function(event, data) {
            that.resolvePromotions();
        });
    };

    ContextHub.Utils.inheritance.inherit(CartStore, ContextHub.Store.PersistedJSONPStore);

    /**
     * Resolve any "${resourcePath}" references in the service path.
     */
    CartStore.prototype.getServiceDetails = function() {
    ...
    /**
     * Override ContextHub.Store.JSONPStore's queryService to support posting of the cart
     * data for re-calculation queries.
     * @param {Boolean} postCart
     */
    CartStore.prototype.queryService = function(reload, postCart) {
        var url = this.getServiceURL(true);
        ...
        var cartData = {ignorePost: true};
        if (postCart) {
            cartData = this.getTree();
        }

        /* setup callback and timeout */
        window.ContextHub = $.extend({ Callbacks: {} }, window.ContextHub);
        ContextHub.Callbacks[this.name] = this.callbackFunction.bind(this);
        var serviceTimeout = this.config.service.timeout;
        var self = this;
        ...
        $.ajax({
            url: url,
            type: "POST",
            data: {cart: ContextHub.Utils.JSON.stringify(cartData)},
            timeout: serviceTimeout
        }).done(function(result, status, xhr) {
            ContextHub.Callbacks[self.name](ContextHub.Utils.JSON.parse(xhr.responseText));
            self.eventing.trigger("push-success:" + self.name);
        }).fail(function(error) {
            self.failureHandler(error);
        }).always(function() {
            if (!wasPaused) {
    ...
    /**
     * Update the store from the server response.  Re-overlay any simulated prices.
     */
    CartStore.prototype.successHandler = function(response) {
    ...
    /**
     * Set a simulated totalPrice into the cart.
     * The simulated price will be kept as long as the underlying (non-simulated) totalPrice doesn't change.
     * @param value
     */
    CartStore.prototype.setSimulatedPrice = function(value) {
        if (!this.getItem("/_/simulatedTotalPrice")) {
            // new simulation; need to store the underlying (reference) totalPrice
            this.setItem("/_/referenceTotalPrice", this.getItem("totalPriceFloat"), {silent: true});
        }
        this.setItem("/_/simulatedTotalPrice", value, {silent: true});

        this.uber("setItem", "totalPriceFloat", value);
        this.setItem("totalPrice", this.getItem("totalPrice").replace(/[0-9]+\.[0-9]+/, parseFloat(value).toFixed(2)));
    };

    /**
     * Apply a simulated totalPrice to the cart if the underlying (non-simulated) totalPrice is still the
     * reference value.
     */
    CartStore.prototype.updateSimulatedPrice = function() {
        var simulatedTotalPrice = this.getItem("/_/simulatedTotalPrice");
        if (simulatedTotalPrice && this.getItem("/_/referenceTotalPrice") == this.getItem("totalPriceFloat")) {
            this.setSimulatedPrice(simulatedTotalPrice);
        } else {
            this.setItem("/_/simulatedTotalPrice", null, {silent: true});
        }
    };

    /**
     * Add a product to the cart.
     * @param productPath The path to the product variant.
     * @param pagePath The path to the product page.
     * @param title The product's title.
     * @param image The product's image url.
     * @param price The product's (formatted) price.
     */
    CartStore.prototype.addProductToCart = function(productPath, pagePath, title, image, price) {
        var entries = this.getItem("entries") || [];
        entries.push({
            title: title,
            quantity: 1,
            path: productPath,
        ...
        this.setItem("entries", entries);

        if (ContextHub.getStore("abandonedproducts")) {
            ContextHub.getStore("abandonedproducts").record(pagePath, title, image, price);
        }
    };
     ...
     * @param quantity
     */
    CartStore.prototype.updateQuantity = function(entryIndex, quantity) {
        var entries = this.getItem("entries") || [];
        if (entries.length >= entryIndex && entries[entryIndex].quantity != quantity) {
     ...
     * @param path
     */
    CartStore.prototype.addSimulatedPromotion = function(path, title) {
        var simulatedPromotions = this.getItem("/_/simulatedPromotions") || [],
            found = false;
        for (var i = 0; i < simulatedPromotions.length; i++) {
            if (simulatedPromotions[i].path == path) {
                found = true;
                break;
            }
        }
        if (!found) {
            simulatedPromotions.push({path: path, title: title});
            this.setItem("/_/simulatedPromotions", simulatedPromotions, {silent: true});
     ...
     * @param path
     */
    CartStore.prototype.removeSimulatedPromotion = function(path) {
        var simulatedPromotions = this.getItem("/_/simulatedPromotions") || [],
            found = -1;
        for (var i = 0; i < simulatedPromotions.length; i++) {
            if (simulatedPromotions[i].path == path) {
        ...
        if (found >= 0) {
            simulatedPromotions.splice(found, 1);
            this.setItem("/_/simulatedPromotions", simulatedPromotions, {silent: true});
            this.resolvePromotions();
        }
    };
     ...
     */
    CartStore.prototype.getSimulatedPromotions = function() {
        return this.getItem("/_/simulatedPromotions");
    };

    /**
     * Add a voucher (by code) to the cart.
     */
    CartStore.prototype.addVoucher = function(voucherCode) {
        var vouchers = this.getItem("vouchers") || [],
            found = false;
        ...
        if (!found) {
            vouchers.push({code: voucherCode});
            this.setItem("vouchers", vouchers);
        }
    };

    /**
     * Remove an existing voucher (identified by its path) from the cart.
     */
    CartStore.prototype.removeVoucher = function(voucherPath) {
        var vouchers = this.getItem("vouchers") || [],
            found = -1;
        for (var i = 0; i < vouchers.length; i++) {
            if (vouchers[i].path == voucherPath) {
                found = i;
                break;
            }
        }
    ...
    /**
     * Override setItem() to defer update events until successHandler.
     * @param key
     * @param value
     * @param options
     */
    CartStore.prototype.setItem = function(key, value, options) {
        if (key.indexOf("entries") == 0 || key.indexOf("promotions") == 0 || key.indexOf("vouchers") == 0) {
            var lookupKey = '/' + (ContextHub.Utils.JSON.tree.sanitizeKey(key) || []).join('/');
            var old = this.getItem(lookupKey);

            /* perform request if old and new value are different */
            var skipRequest = old === value;

            /* stringify if it's object */
            if (!skipRequest) {
                var oldVal = ContextHub.Utils.JSON.stringify(old);
                var newVal = ContextHub.Utils.JSON.stringify(value);

                skipRequest = oldVal === newVal;
            }

            if (skipRequest === false) {
                var callQueryService = function() {
                    this.queryService(false, true);
                }.bind(this);

                ContextHub.eventing.once(ContextHub.Constants.EVENT_STORE_UPDATED + ':cart', callQueryService, 'commerceRemoteSync');
            }
        }
        this.uber("setItem", key, value, options);
    };

    /**
     * Return whether the cart contains a particular product or not.
     * @param product The product page path (may contain the variation hash as well, which will be ignored)
     * @param minQuantity
     * @returns {boolean}
     */
    CartStore.prototype.containsProduct = function(product, minQuantity) {
        var productPagePath = product ? mainPart(product) : null,
            entries = this.getItem("entries") || [];
        if (!minQuantity) {
            minQuantity = 1;
        }
        for (var i = 0; i < entries.length; i++) {
            var entry = entries[i];
            var entryPagePath = entry.page || entry.pagePath || '';
            entryPagePath = mainPart(entryPagePath);
            if ((!product || entryPagePath == productPagePath) && (!minQuantity || entry.quantity >= minQuantity)) {
                return true;
            }
        }
        return false;
    ...
    CartStore.prototype.containsPromotion = function(promotion, status) {
        var promotionPath = promotion ? mainPart(promotion) : null,
            promotions = this.getItem("promotions") || [];
        if (!status) {
            status = 'FIRED';
        }
        for (var i = 0; promotions && i < promotions.length; i++) {
            var cartPromotion = promotions[i];
            var entryPath = cartPromotion.path ? mainPart(cartPromotion.path) : null;
    ...
    /**
     * Resolve promotions against a promotions-to-segments map.
     */
    CartStore.prototype.resolvePromotions = function() {
        var promotionsMap = this.getItem("promotionsMap");
        if (!promotionsMap) {
            return;
        }

        //
        // Resolve promotions against the resolved segments via the promotions-to-segments map:
        //
        var resolvedSegments = ContextHub.SegmentEngine.SegmentManager.getResolvedSegments({ returnLookup: true }),
            resolvedPromotions = {},
            i, j;
        for (i = 0; i < promotionsMap.length; i++) {
            var testSegments = promotionsMap[i].segments.split(",");
            for (j = 0; j < testSegments.length; j++) {
                if (resolvedSegments[testSegments[j]]) {
                    resolvedPromotions[promotionsMap[i].path] = true;
                    break;
                }
            }
        }
        ...
        //
        // Add in any simulated promotions:
        //
        var simulatedPromotions = this.getItem("/_/simulatedPromotions") || [];
        for (i = 0; i < simulatedPromotions.length; i++) {
            resolvedPromotions[simulatedPromotions[i].path] = true;
        }

        //
        // Update the store's list of promotions:
        //
        var promotions = this.getItem("promotions") || [];
        for (i = 0; i < promotions.length; i++) {
            if (promotions[i].resolve                                   // resolved on server
                    || resolvedPromotions[promotions[i].path]) {        // resolved on client
                resolvedPromotions[promotions[i].path] = undefined;
            } else {
                promotions.splice(i--, 1);
            }
        }
        for (var newlyResolvedPath in resolvedPromotions) {
            if (resolvedPromotions.hasOwnProperty(newlyResolvedPath)) {
                promotions.push({path: newlyResolvedPath});
            }
        }
        this.setItem("promotions", promotions);
    };

    CartStore.prototype.reset = function(keepRemainingData) {
        this.uber("reset", keepRemainingData);
        //skipping sync functionality in overriden setItem()
    ...
    ContextHub.Utils.storeCandidates.registerStoreCandidate(CartStore, 'commerce.cart', 0);
...
(function ($) {
    ...
    var RelatedProductsStore = function(name, config) {
        this.config = $.extend(true, {}, defaultConfig, config);
        this.init(name, this.config);
        ...
        /* re-run query if product/page, user or cart is updated */
        ContextHub.eventing.on(ContextHub.Constants.EVENT_STORE_UPDATED + ":profile", function(event, data) {
            var authorizableId = ContextHub.getStore("profile").getItem("authorizableId");

            if (!that.profileLoaded) {
                that.profileLoaded = true;
                that.lastAuthorizableId = authorizableId;
            } else {
                if (authorizableId != that.lastAuthorizableId) {
                    that.lastAuthorizableId = authorizableId;
                    that.queryService(true);
                }
            }
        ...
        ContextHub.eventing.on(ContextHub.Constants.EVENT_STORE_UPDATED + ":cart", function(event, data) {
            var cartEntries = ContextHub.getStore("cart").getItem("entries") || [];
            var cartEntryCount = cartEntries.length;

            if (!that.cartLoaded) {
                that.cartLoaded = true;
                that.lastCartEntryCount = cartEntryCount;
            } else {
                if (cartEntryCount !== that.lastCartEntryCount) {
                    that.lastCartEntryCount = cartEntryCount;
                    that.queryService(true);
                }
            }
    ...
    /* inherit from ContextHub.Store.JSONPStore */
    ContextHub.Utils.inheritance.inherit(RelatedProductsStore, ContextHub.Store.JSONPStore);

    /**
     * Stores jsonp response in the persistence.
     *
     * @param {Object} response - jsonp response
     */
    RelatedProductsStore.prototype.successHandler = function(response) {
        this.setItem('/', response || {});
    };

    /**
     * Get relationships of a particular type.
     */
    RelatedProductsStore.prototype.products = function(maxCount, relationshipType) {
        var storeTypes = this.getTree();
        var typeList = null;
        var result = [];

        for (var i = 0; i < storeTypes.length; i++) {
            if (storeTypes[i].type === relationshipType) {
                typeList = storeTypes[i].list;
                break;
            }
        }

        for (i = 0; (typeList && i < typeList.length) && (i < maxCount); i++) {
    ...
    ContextHub.Utils.storeCandidates.registerStoreCandidate(RelatedProductsStore, 'commerce.relatedproducts', 0);
